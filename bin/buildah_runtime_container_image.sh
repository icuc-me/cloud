...
build_layer() {
    ...
    set -x
    sudo buildah run --terminal --add-history $container -- /usr/src/$SCRIPT_SUBDIR/$SCRIPT_FILENAME
...
then
    ...
    if ((CHANGED)) || ! sudo podman images $IMAGE_BASE:$IMAGE_TAG &> /dev/null
    then
        sudo podman rmi $IMAGE_BASE:$IMAGE_TAG 2> /dev/null || true
        trap cleanup EXIT
        sudo podman tag $IMAGE_BASE:$_LAYER_3 $IMAGE_BASE:$IMAGE_TAG
        trap - EXIT
    else
